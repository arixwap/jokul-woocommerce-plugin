...
require_once(DOKU_JOKUL_PLUGIN_PATH . '/Service/JokulBriVaService.php');
require_once(DOKU_JOKUL_PLUGIN_PATH . '/Common/JokulDb.php');

class JokulBriVaModule extends WC_Payment_Gateway
{
    public function __construct()
    {
        ...
        $this->method_name          = 'Bank Rakyat Indonesia';
        $this->method_code          = 'VIRTUAL_ACCOUNT_BANK_BRI';
        ...
        add_action('woocommerce_thankyou_' . $this->id, array($this, 'thank_you_page_bri_va'), 1, 10);
    }

    public function init_form_fields()
    {
        $this->form_fields = require(DOKU_JOKUL_PLUGIN_PATH . '/Form/JokulBriVaSetting.php');
    }
...
?>
    ...
    public function process_payment($order_id)
    {
        ...
        $this->briVaService = new JokulBriVaService();
        $response = $this->briVaService->generated($config, $params);
        if (!is_wp_error($response)) {
            ...
            if (!isset($response['error']['message']) && isset($response['virtual_account_info']['virtual_account_number'])) {
                ...
                JokulBriVaModule::addDb($response, $amount, $order, $vaNumber, $vaExpired, $processType);
    ...
    public function thank_you_page_bri_va($order_id)
    {
        $vaNumber       = get_post_meta($order_id, 'jokul_va_number', true);
        $vaExpired      = get_post_meta($order_id, 'jokul_va_expired', true);
