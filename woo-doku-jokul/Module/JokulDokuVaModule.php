...
class JokulDokuVaModule extends WC_Payment_Gateway
{
    public function __construct()
    {
        ...
        $this->id                   = 'jokul_dokuva';
        $this->has_fields           = true;
        $this->method_name          = 'Other Banks (VA by DOKU)';
        $this->method_code          = 'VIRTUAL_ACCOUNT_DOKU';
        ...
        add_action('woocommerce_thankyou_' . $this->id, array($this, 'thank_you_page_doku_va'), 1, 10);
    }

    public function init_form_fields()
    {
        $this->form_fields = require(DOKU_JOKUL_PLUGIN_PATH . '/Form/JokulDokuVaSetting.php');
    }
...
?>
    ...
    public function thank_you_page_doku_va($order_id)
    {
        $vaNumber       = get_post_meta($order_id, 'jokul_va_number', true);
        $vaExpired      = get_post_meta($order_id, 'jokul_va_expired', true);
